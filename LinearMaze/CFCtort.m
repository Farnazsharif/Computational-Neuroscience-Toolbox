function [tort,freq,MI] = CFCtort(LFPtheta,signals,frecs1,frecs2,Fs,band,makePlot)
...
%canolty=zeros(length(frecs1),length(frecs2),Nsenalesamp);
...
for if1=1:length(frecs1)  
    ...
    for if2=1:length(frecs2) 
        for isen=1:Nsenalesamp
            ...
            %canolty(if1,if2,:)=squeeze(mean(amp2(if2,:,:).*exp(1i*fase1(if1,:,:)),2));
         end
    end
end
...
contourf(frecs1,frecs2,dibujar',20,'LineColor','none');
