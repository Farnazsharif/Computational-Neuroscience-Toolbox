...
% Fs=1000;
% phaseBand=[2 3 5 6];
% band=[30 200];LFP=eeg(:,Ch(D));
% LFP=eeg(:,Ch(D));
% LFPtheta=lfp(Bin_beging(tr,1):Bin_end(tr,100));
% signals=LFPtheta;
...
for j=1:length(plotsignals)
    ...
    PP=cmats_sig(:,[1:end 1:end],j,1);
    S=conv2(Smoother, Smoother,PP, 'same');
%     imgs(j,1) = imagesc([midpoints(xtheta(1:2)) 4*pi-midpoints(xtheta(1:2))]*180/pi, fspec(plotfreqs([1 end])), S, clims);
    imgs(j,1) = imagesc([midpoints(xtheta(1:2)) 4*pi-midpoints(xtheta(1:2))]*180/pi, fspec(plotfreqs([1 end])), S);
