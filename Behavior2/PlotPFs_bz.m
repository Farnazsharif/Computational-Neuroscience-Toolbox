...
for i = 1:size(spikes.times,2)
    ...
    title(['Cell# = ' num2str(i) ' G=' num2str(spikes.UID(i))])  
...
N_image_slide=4;
a=170;% sapce between the figures
b1=-5;%first raw onset
b2=350;%second raw onset
SW=800; % Width of the figures
SL=200;  % Length of the figures
N_slide=ceil(size(spikes.times,2)./N_image_slide);
Shape_position_1=[0*a,b1,SL,SW;a*1,b1,SL,SW;a*2,b1,SL,SW;a*3,b1,SL,SW]
...
%     Image = Slide(k).Shapes.AddPicture([Fullpath '\cell_' num2str(A) '.jpg'], 'msoFalse', 'msoTrue', Shape_position(i,1),Shape_position(i,2),Shape_position(i,3),Shape_position(i,4))  
      Image = Slide(k).Shapes.AddPicture([Fullpath '\Phasecell_ON' num2str(A) '.jpg'], 'msoFalse', 'msoTrue', Shape_position(i,1),Shape_position(i,2),Shape_position(i,3),Shape_position(i,4))   
%             Image = Slide(k).Shapes.AddPicture([Fullpath '\Phasecell_Off' num2str(A) '.jpg'], 'msoFalse', 'msoTrue', Shape_position(i,1),Shape_position(i,2),Shape_position(i,3),Shape_position(i,4))
...
Presentation.SaveAs([Fullpath '\' filename 'ON' '.ppt'])
