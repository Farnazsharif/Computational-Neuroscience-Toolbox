...
for ses =1:length(sessions)%[2 5 7],[1 3 4 6 8]%6:8
    ...
    if ses==2 | ses==5 | ses==7
        ...
        tic
        dataPP_all=[];statsPP_all=[];
        for j=1:(Seg)
            [dataPP_all{1,j},statsPP_all{1,j}]= PhasePrecession_CirularLinear(filename,Rate_Matrix,BinSize,tr(j,:),smooth_rate,delta,speed_thr);
        end
        [dataPP_all{1,(Seg+1)},statsPP_all{1,(Seg+1)}]= PhasePrecession_CirularLinear(filename,Rate_Matrix,BinSize,TR_all,smooth_rate,delta,speed_thr);
        toc
        
        cd (folder)
        save(['dataPP_all_1'],'dataPP_all')
        save(['statsPP_all_1'],'statsPP_all')
        cd ..
        ...
        cd (folder)
        save(['dataPP_all_2'],'dataPP_all')
        save(['statsPP_all_2'],'statsPP_all')
        cd ..
        ...
        cd (folder)
        save(['dataPP_all_3'],'dataPP_all')
        save(['statsPP_all_3'],'statsPP_all')
        cd ..
