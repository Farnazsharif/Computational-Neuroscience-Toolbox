...
function loadbehav_RDH_farn(filename,samplerate)
% clear
% filename='SFA5_S9';
% samplerate=30000;
...
DIN = 0;
ndx = 16-DIN;
reward_time= time_change(find(diff(str2num(digital_array(:,ndx)))~=0)+1);
...
if ~isempty(TRDfilename)
    ...
    txlrts = make_txlrts_fromRDH_Rewardchanging( TX,reward_time );  % or make_txlrts_fromRDH( TX )    
end
'step 3 done'
%%
...
save([filename '.mat'],'-append','behav','spkinfo')
