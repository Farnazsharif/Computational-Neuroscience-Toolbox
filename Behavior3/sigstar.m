function varargout=sigstar(groups,stats,nosort,THR)
    ...
    y=ylim;
    yd=myRange(y)*0.067; %separate sig bars vertically by 5% 

    for ii=1:length(groups)
        thisY=findMinY(xlocs(ii,:))+yd;
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii),THR);
    end
...
function H=makeSignificanceBar(x,y,p,THR)
    ...
    elseif p<=THR %% changed by Farnaz 1 
        stars='*';
    elseif isnan(p)
        stars='n.s.';
    else
    ...
    H(2)=text(mean(x(:)),starY,stars,...
        ...
        'Tag','sigstar_stars','FontSize',22,'Color','k');
