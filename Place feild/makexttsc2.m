...
%function xttsc=makexttsc(TXDTS,spk,xbinNumber,SpeedThreshold)
%
%xttsc=[xbin TimeSpent trialnb setnb nspike];
%SpeedThreshold in cm/s


function xttsc=makexttsc2(TXDTS,spk,CellN,xbinNumber,SpeedThreshold)
d=1; %cm for 1 x increment 

% load('txdtsc700.mat')
% txdtsc=txdtsc700;
% txdtsc=maketxdtsc(TXDTS,spk.ts,spk.g);
txdtsc=maketxdtsc2(TXDTS,spk.ts,spk.g,CellN);
 
ncell=length(txdtsc(1,:))-5;
