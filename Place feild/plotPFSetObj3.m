function plotPFSetObj3(filename,shankID,ndx_todisplay)
%this Function use the clu file to plot the fields, so if we change some
%cluse with clustering then we can use this program
...
load([filename '.mat'])
samplingrate=spkinfo.samplerate;
filename2='FM05_1_clu_';
% ndx_todisplay=3;
% shankID=9;
load([filename2,num2str(shankID),'.mat'])

spk.g = clu.g(ismember(clu.g,ndx_todisplay));
T = Loadres([filename '.res.' num2str(shankID)]);
spk.ts = T(ismember(clu.g,ndx_todisplay))/samplingrate;

nPFbin=100;
SpeedThreshold=5;
TXDTS=behav.TXDTS;
xttsc=makexttsc(TXDTS,spk,nPFbin,SpeedThreshold);

%smooth
smoothT=smooth1D(xttsc(:,2),10,1);
smoothC=smooth1D(xttsc(:,5),10,1);
